...
class FPLSession:
    ...
    def set_current_gameweek(self):
        for gw in self.gameweeks:
            if gw and gw.is_current:
                log.info("Current gameweek: {}".format(gw.id))
                self.curr_gameweek = gw.id
            if gw and gw.is_next:
                log.info("Next gameweek: {}".format(gw.id))
    ...
    def is_valid_gameweek_fixture(self, fixture):
        if (
            ...
            log.info(
                "Match week {} still in progress. {} vs {}".format(
    ...
    async def fpl_get_fixtures(self):
        ...
        for i in range(0, self.curr_gameweek):
            ...
            if not self.is_valid_fixtures(fixtures):
                ...
                fixtures = await self.h2h_league.get_fixture(str(gameweek) + "&page=1")
                if not self.is_valid_fixtures(fixtures):
    ...
    async def fpl_get_session(self):
        async with aiohttp.ClientSession() as session:
            ...
            custom_cookie = {
                "pl_profile": os.environ["FPL_PL_PROFILE"],
                "datadome": os.environ["FPL_DATADOME"],
            }

            await self.fpl_session.login(cookie=custom_cookie)
            ...
            try:
                ...
                log.info(("Failed to retrieve game week data, " "try second method"))
                # TODO - Refactor
                try:
                    self.h2h_league_all_fixtures = await self.fpl_get_fixtures_2()
                    self.fpl_fixtures_retrieve_method = 2
                except ValueError:
                    log.info(("Failed to retrieve game week data, " "try third method"))
    ...
    def fpl_get_h2h_league_fixtures(self):
        if (
            self.fpl_fixtures_retrieve_method == 1
            or self.fpl_fixtures_retrieve_method == 3
        ):
            # List of lists
        ...
        log.info(
            "Build H2H league info method {0}".format(self.fpl_fixtures_retrieve_method)
        )
    ...
    def is_gameweek_data_checked(self):
        ...
        if gw_obj.id != self.curr_gameweek:
            log.error(
                "is_gameweek_data_checked: Wrong gameweek! {} {}".format(
                    gw_obj.id, self.curr_gameweek
                )
            )
            sys.exit(2)
